...
**• Avantajları:**  
**o Fiziksel Benzerlik:** Kümeleme, fiziksel olarak benzer partilerin gruplandırılması kavramıyla uyumludur.  
**o Esneklik:** Bir partinin süreç içindeki ilerlemesiyle tahmin edicilerin sayısındaki değişimi kolayca yönetir.  

**• Dezavantajları:**  
**o Kullanıcı Etkileşimi:** Optimal küme sayısını belirlemek için kullanıcı müdahalesi gereklidir.  
**o Güncelleme İhtiyacı:** Ürün veya fabrika anahtar özellikleri değiştikçe kümelerin güncellenmesi gerekir.  
...
**• En Düşük Hata Oranı:** Çalışmada, "Regresyon Ağaçları (CART)" algoritmasının en düşük medyan mutlak hatayı (0.042) ve ortalama mutlak hatayı (0.09) ürettiği tespit edilmiştir.  
**• Kümeleme ile Kombinasyon:** CART'ın "Kümeleme" ile birleştirildiğinde (CART in Cluster) performansının daha da iyileştiği, medyan mutlak hatanın 0.036'ya ve ortalama mutlak hatanın 0.06'ya düştüğü gözlemlenmiştir. Bu kombinasyon, diğer yöntemlere kıyasla en düşük hata oranlarını sağlamıştır.  
**• KNN ve Sinir Ağları Performansı:** K-En Yakın Komşular (KNN) yönteminin, K=5 için 0.070 medyan mutlak hata ve K=10 için 0.078 medyan mutlak hata ile regresyon ağaçlarından daha yüksek hata oranlarına sahip olduğu belirtilmiştir. Sinir Ağları'nın medyan mutlak hatası ise 0.085 olarak kaydedilmiştir.  
#### Önerilen Çözüm ve Robustluk
**• Regresyon Ağaçlarının Avantajları:** Regresyon ağaçları, hem kategorik hem de sürekli tahmin edici değişkenleri iyi bir şekilde ele alabilen esnek bir araç sunar. Ayrıca, önemli değişkenleri tanımlama ve partiler arasındaki benzerliği belirleme yeteneği açısından avantajlıdır.  
**• Aykırı Değer Yönetimi:** Üretim ortamlarında yaygın olan gürültülü veriler için, regresyon ağacı tahmincisinin robustluğu ele alınmıştır. Düğüm ortalaması yerine düğüm medyanının kullanılması, tahminleri iyileştirmiştir.  
**• Model Güncelleme Kolaylığı:** Karar ağacı modelinin bir diğer avantajı, proses değişikliklerini yansıtmak üzere hızlı ve kolay bir şekilde yeniden oluşturulabilmesidir.  
